...
--------------------------------------------------
    ...
    def check_cheating(self, roll_value):
        ...
        if roll_value == "a":
            lock_value = self.dice_b.currentValue
        else:
            lock_value = self.dice_a.currentValue

        if self.currentStage == 1:
            if lock_value in self.possibleValues[2:]:
                print("That's cheating!")
                self.cheating = True

        elif self.currentStage == 2:
            if lock_value in (self.possibleValues[:2] + self.possibleValues[4:]):
                print("Hey cheater!")
                self.cheating = True

        elif self.currentStage == 3:
            if lock_value in (self.possibleValues[:4]):
                print("That's cheating!")
                self.cheating = True
            elif lock_value == "6":
